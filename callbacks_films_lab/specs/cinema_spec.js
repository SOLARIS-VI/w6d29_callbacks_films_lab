...
describe('Cinema', function () {
  ...
  it('should be able to filter films by genre', function () {
    const actual = cinema.filmsByGenre('sci-fi');
    const expected = [bladeRunner];
    assert.deepStrictEqual(actual, expected);

    const actual2 = cinema.filmsByProperty('genre', 'sci-fi');
    assert.deepStrictEqual(actual2, expected);
  });
  

  it('should return true if there are some films from a particular year', function () {
    const actual = cinema.someFilmsFromYear(2018);
    assert.strictEqual(actual, true);
  }); 
  

  it('should be able to check whether there are no films from a particular year', function () {
    const actual = cinema.noFilmsFromYear(2018);
